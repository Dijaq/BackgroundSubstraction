...
gt = np.float32(list(map(lambda x: cv2.imread(x, cv2.IMREAD_GRAYSCALE), gt))) / 255.0
	...
	for i in range(H):
		for j in range(W):
			...
			for k in range(1, int(R)):
				samples[k,i,j] = frame[int(i0)+random.randrange(-int(R), int(R)+1),int(j0)+random.randrange(-int(R), int(R)+1)]
